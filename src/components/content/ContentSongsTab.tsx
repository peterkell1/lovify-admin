...
import { Search, Play, Pause, Eye, EyeOff, Music, Trash2 } from 'lucide-react'
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteContent.mutateAsync({
        ...
        id: confirmDelete.id,
        metadata: { title: confirmDelete.title },
      })
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="relative aspect-square bg-muted">
                ...
                <div className="absolute top-1.5 right-1.5 flex gap-1">
                  <button
                    ...
                    className={`h-7 px-2 rounded-md shadow-sm flex items-center justify-center gap-1 text-[10px] font-semibold cursor-pointer disabled:opacity-50 transition-colors ${song.is_public ? 'bg-success/90 text-white hover:bg-destructive/90' : 'bg-white/90 text-tertiary hover:bg-success/90 hover:text-white'}`}
                  >
                    {song.is_public ? <><Eye className="h-3 w-3" /> Live</> : <><EyeOff className="h-3 w-3" /> Off</>}
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); setConfirmDelete({ id: song.id, title: song.title || 'Untitled' }) }}
      ...
      <ConfirmDialog
        ...
        description={`"${confirmDelete?.title ?? ''}" will be permanently removed from the database. The creator will lose access to it. This action CANNOT be undone.`}
