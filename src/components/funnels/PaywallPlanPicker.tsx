        ...
        <div className="border-t border-border pt-3 space-y-1">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1">
              <Select
                value={mostPopularPlanKey ?? ''}
                onChange={(e) => onMostPopularChange(e.target.value || null)}
                disabled={value.length === 0}
              >
                <option value="">
                  {defaultPlanKey
                    ? `Default (${value.find((p) => p.planKey === defaultPlanKey)?.label ?? defaultPlanKey})`
                    : 'Same as default plan'}
                </option>
                {value.map((p) => (
          ...
          <p className="text-[11px] text-muted-foreground">
            Card with the "MOST POPULAR" ribbon in templates that surface it. Defaults to the default plan.
          </p>
        </div>
      </div>
